...
/// <summary>
/// 菜单种子（独立文件）。
/// 从原 InfoSeeds 中拆出，便于单独维护菜单而不影响其他种子数据。
/// 新增菜单后运行 Debug → “写入菜单/字典” 即可幂等入库（无需重新迁移）。
/// </summary>
public static class MenuSeeds
{
    public static void Seed(ModelBuilder mb)
    {
        ...
        mb.Entity<SysMenu>().HasData(
            ...
            // ── 资讯公告（Info 模块）──────────────────────────
            new SysMenu { Id = 41,  MenuName = "资讯公告", ParentId = 0,  MenuType = "M", Icon = "fa-bullhorn", Path = "/info/manage", Sort = 4, Visible = 1, Status = 1, CreatedAt = dt, CreatedBy = "system" },
            new SysMenu { Id = 411, MenuName = "公告管理", ParentId = 41, MenuType = "C", Icon = "fa-bullhorn", Path = "/info/manage", Sort = 1, Visible = 1, Status = 1, Perms = "info:article:list", CreatedAt = dt, CreatedBy = "system" },
            new SysMenu { Id = 412, MenuName = "分类管理", ParentId = 41, MenuType = "C", Icon = "fa-tags", Path = "/info/category", Sort = 2, Visible = 1, Status = 1, Perms = "info:category:list", CreatedAt = dt, CreatedBy = "system" },
            new SysMenu { Id = 413, MenuName = "新增", ParentId = 411, MenuType = "F", Sort = 1, Visible = 0, Status = 1, Perms = "info:article:add", CreatedAt = dt, CreatedBy = "system" },
            new SysMenu { Id = 414, MenuName = "删除", ParentId = 411, MenuType = "F", Sort = 2, Visible = 0, Status = 1, Perms = "info:article:delete", CreatedAt = dt, CreatedBy = "system" }
        );
    }
}
