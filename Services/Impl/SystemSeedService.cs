...
public class SystemSeedService : ISystemSeedService
{
    ...
    public async Task<List<string>> SeedMenuAndDictsAsync()
    {
        ...
        var menuSeeds = new List<SysMenu>
        {
        ...
        foreach (var mid in superAdminMenuIds)
        {
            ...
            {
                _db.SysRoleMenus.Add(new SysRoleMenu { RoleId = 1, MenuId = mid });
                rmAdded++;
            }
            else rmSkipped++;
        }
        ...
        // ── 字典类型 / 字典数据：同样改为直接插入 ──────────────
        var dictTypeSeeds = new List<SysDictType>
        {
        ...
        var dictDataSeeds = new List<SysDictData>
        {
